...
public:
    ...
    std::vector<float> operator[](const std::vector<int64_t> &idx) const override;

    // void set(const torch::Tensor &idx, const torch::Tensor &value) override;
    void set(const std::vector<int64_t> &idx, const std::vector<float> &value) override;
    ...
    [[nodiscard]] std::vector<int64_t> get_prefix_sum_idx(const std::vector<float> &value) const override;

    // void get_prefix_sum_idx_sycl(queue &q, torch::Tensor value, IntVector &index_parallel);
    void get_prefix_sum_idx_sycl(queue &q, std::vector<float> value, IntVector &index_parallel);

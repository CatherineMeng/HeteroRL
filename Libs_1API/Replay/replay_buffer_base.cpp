...
// add data samples
//manual check: should always be associated with priority sum tree insertion (set) when called
void ReplayBuffer::add_batch(str_to_tensor &data) {
    int64_t batch_size = data.begin()->second.sizes()[0]; //data should contain one pair. tensor 0st dimension is the batch size
    if (m_ptr + batch_size > capacity()) {
        std::cout << "Reaches the end of the replay buffer" << std::endl; //FIFO
    }
    for (auto &it: data) {
        AT_ASSERT(batch_size == it.second.sizes()[0]);
        if (m_ptr + batch_size > capacity()) { //wrap-around, check

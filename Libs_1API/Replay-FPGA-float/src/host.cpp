...
using ConsumePipe = ext::intel::pipe<class ConsumePipeClass, sibit_io,16>;
  ...
  for (size_t i = 0; i < iterations; i++) {
    ...
    do {
      ...
      if (in_chunk < chunks) {
        ...
        event p_e = Submit_Producer_SiblingItr<ProducePipe>(q, in + chunk_offset, x,
                                                   chunk_size);
        event c_e = Submit_Consumer_SiblingItr<fixed_l3, fixed_l2, ConsumePipe, Lev3_Width>(q, sampled_idx + chunk_offset,
    ...
    //   e.wait();
    // }
    events.wait(); //only one event, not an array for testbench
...
void Init_Tree(sycl::queue& q, fixed_root &root_pr){
    ...
    std::queue<std::pair<event,event>> clear_q;
    std::swap(event_q, clear_q);
    auto events = Submit_Intermediate_SiblingItr<Itm1,fixed_l2,fixed_l3,ProducePipe,ConsumePipe,2,16>(q, chunk_size*chunks);
    while(out_chunk < chunks){
      ...
      if (in_chunk < chunks) {
        // launch the producer/consumer pair for the next chunk of data
        size_t chunk_offset = in_chunk*chunk_size;
        event p_e = Submit_Producer_SiblingItr<ProducePipe>(q, in + chunk_offset, 0,chunk_size);
        event c_e = Submit_Consumer_SiblingItr<fixed_l3, fixed_l2, ConsumePipe, Lev3_Width>(q, sampled_idx + chunk_offset,
                                                   out_pr_sampled + chunk_offset,
                                                   out_pr_insertion + chunk_offset,
                                                   chunk_size);

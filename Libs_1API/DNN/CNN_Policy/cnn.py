...
class Trainer:
    ...
    def compute_td_loss(self, batch_size, gamma):
        ...
        for _ in range(batch_size):
            s_tensor = torch.rand((1, 3, 210, 160), requires_grad=True)
            ns_tensor = torch.rand((1, 3, 210, 160), requires_grad=True)
    ...
    def train(self, random_seed, rom_path, num_epochs, num_actions):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if random.random() <= epsilon:
                ...
                state_tensor = torch.rand((1, 3, 210, 160))  # Replace with actual environment observation later.
            ...
            next_state_tensor = torch.rand((1, 3, 210, 160))  # Replace with actual environment observation later.

            reward_tensor = torch.tensor(reward)
            done_tensor = torch.tensor(done, dtype=torch.float32)
            action_tensor_new = torch.tensor(action, dtype=torch.long)
            ...
            if done:
                state_tensor = torch.rand((1, 3, 210, 160))  # Renew state if done.
            ...
            if epoch % 1000 == 0:
                ...
                # self.loadstatedict(self.network, self.target_network)

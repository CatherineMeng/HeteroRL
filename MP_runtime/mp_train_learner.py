...
def actor_process(param_conn, actor_id, data_collection_conn):
    ...
    while True:
        # rollout_cnt+=1
        # if (rollout_cnt>1):
        # print("=== actor", actor_id,"doing epiode")
        state = env.reset()[0]
        # print("  state :", state )
        done = False
        # run one episode 
        ...
        while not done:
            ...
            state = next_state
        # print("***=== Actor", actor_id,"reward =",rewards)
...
def learner_process(param_conns, data_transfer_conn, batch_size, gamma, use_gpu):
    ...
    # for _ in range(num_training_eps):
        ...
        if (learn_itr_cnt<=num_training_eps):
            # print("Learner itr",learn_itr_cnt,"starts")
            policy_net.to(device)
            ...
            loss.backward()
            optimizer.step()
            # print("Learner itr",learn_itr_cnt,"here")

            if learn_itr_cnt % 10 == 0:  # Update the target network every 10 episodes

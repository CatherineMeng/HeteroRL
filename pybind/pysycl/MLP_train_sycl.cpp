...
struct params_out {
    params_out() {
        hiddenWeights_d.resize(L1);
        for (size_t i=0; i<L1; i++)hiddenWeights_d[i].resize(L2);
        hiddenBiases_d.resize(L2);
        outputWeights_d.resize(L2);
        for (size_t i=0; i<L2; i++)outputWeights_d[i].resize(L3);
        outputBiases_d.resize(L3);  
    }
    std::vector<std::vector<float>> hiddenWeights_d;
    std::vector<float> hiddenBiases_d;
...
class DQNTrainer {     
  public:  
    ...
    params_out updated_params(){
      ...
      for (size_t i=0; i<L2; i++){
        param_pack.hiddenBiases_d[i] = hiddenBiases[i];
      }
      for (size_t i=0; i<L3; i++){
      ...
      for (size_t i=0; i<L1; i++){
        for (size_t j=0; j<L2; j++){
          param_pack.hiddenWeights_d[i][j] = hiddenWeights[i][j];
        }
      }
      for (size_t i=0; i<L2; i++){
        for (size_t j=0; j<L3; j++){
...
int main() {
  ...
  std::vector<float> hb;
  hb.resize(L2);
  for (size_t i = 0; i < L2; i++){hb[i]=0.1;}
  ...
  std::vector<float> ob;
  ob.resize(L3);
  for (size_t i = 0; i < L3; i++){ob[i]=0.2;}
